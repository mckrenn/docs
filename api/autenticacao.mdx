...
```bash CURL
curl -X POST "https://api.gohusky.net/createStore?token=SUA_TOKEN_OPERACAO&customer=false"
```

```javascript JavaScript
const response = await fetch('https://api.gohusky.net/createStore?token=SUA_TOKEN_OPERACAO&customer=false', {
  method: 'POST'
});
```
...
```bash CURL
curl -X POST "https://api.gohusky.net/createOrderList?token=SUA_TOKEN_EMBARCADOR&customer=true"
```

```javascript JavaScript
const response = await fetch('https://api.gohusky.net/createOrderList?token=SUA_TOKEN_EMBARCADOR&customer=true', {
  method: 'POST'
});
```
...
```bash CURL
curl -X POST "https://api.gohusky.net/v2/getOrderStatus" \
  -H "Token: SUA_TOKEN_EMBARCADOR"
```

```javascript JavaScript
const response = await fetch('https://api.gohusky.net/v2/getOrderStatus', {
  method: 'POST',
  headers: {
    'Token': 'SUA_TOKEN_EMBARCADOR'
  }
